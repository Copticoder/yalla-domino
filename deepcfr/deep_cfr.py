...
class ReservoirBuffer(object):
  ...
  def add(self, elements):
    """Potentially adds elements to the reservoir buffer.

    Args:
      elements: A single element or list of elements to be added to the reservoir buffer.
    """
  ...
  def solve(self):
    ...
    for i in tqdm(range(self._num_iterations), desc="Training iterations"):
      for p in tqdm(range(self._num_players), desc=f"Players (iteration {i+1})", leave=False):
        for _ in tqdm(range(self._num_traversals), desc=f"Traversals (player {p+1})", leave=False):
          self._traverse_game_tree(self._root_node, p)
        if self._reinitialize_advantage_networks:
          # Re-initialize advantage network for player and train from scratch.
          self.reinitialize_advantage_network(p)
        # Re-initialize advantage ne tworks and train from scratch.
