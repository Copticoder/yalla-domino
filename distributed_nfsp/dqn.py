...
class DQN(rl_agent.AbstractAgent):
  ...
  def __init__(self,
    ...
    # the target q network has the same parameter initializations as the q network
  ...
  def learn(self):
    ...
    # -----------------------------------------------------------------------
    # Exit early if we do not have enough data to learn from.
    # -----------------------------------------------------------------------
    if (
    ...
    legal_actions_mask = torch.tensor(np.asarray([t.legal_actions_mask for t in transitions]))

    # -----------------------------------------------------------------------
    # Forward pass – compute current Q-values and target Q-values.
    # -----------------------------------------------------------------------
    self._q_values = self._q_network(info_states)
    with torch.no_grad():
      self._target_q_values = self._target_q_network(next_info_states)
    ...
    targets                = rewards + (1 - are_final_steps) * self._discount_factor * max_next_q

    batch_indices          = torch.arange(self._q_values.size(0))
    predictions            = self._q_values[batch_indices, actions]
