...
def parseStateAsModelInput(state):
    ...
    hand.sort()
    hand += [0] * (21 - len(hand))
    # Fill board sequence to length 28
...
class SLModel(torch.nn.Module):
    ...
    def __init__(self, **kwargs):
        ...
        self.backbone = torch.nn.Sequential(
            ...
            torch.nn.LeakyReLU(),
            torch.nn.BatchNorm1d(self.inner_dim),
            torch.nn.Linear(self.inner_dim, self.cards_total_num * 2)  # Connect to the left or right side
        )
    ...
    def forward(self, data):
        ...
        return self.backbone(torch.flatten(features, 1))
